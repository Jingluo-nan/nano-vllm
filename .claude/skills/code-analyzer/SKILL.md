...
1. **读 `CLAUDE.md`**（如果存在）——里面已有大部分架构信息，作为基础。
2. **快速校验关键文件存在且未大幅变动**：用 Read 抽查 `engine/llm_engine.py`、`engine/scheduler.py`、`engine/model_runner.py`、`layers/attention.py` 的关键函数签名。CLAUDE.md 可能过时——以代码为准。
